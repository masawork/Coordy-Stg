...
function ResetPasswordContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('📤 パスワードリセット実行（サービス出品者）:', { email, code });
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-600 via-emerald-500 to-teal-400 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="mt-6 text-center space-y-2">
          <Link href="/login/instructor/forgot" className="text-green-600 hover:text-green-700 font-semibold block">
            コードを再送信する
          </Link>
          <Link href="/login/instructor" className="text-gray-500 hover:text-gray-700 block">
            ログイン画面に戻る
          </Link>
        </div>
      </motion.div>
