...
export default function InstructorForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('📤 パスワードリセットメール送信（サービス出品者）:', email);
      ...
      // 2秒後にリセットコード入力画面へ遷移
      setTimeout(() => {
        router.push(`/login/instructor/reset?email=${encodeURIComponent(email)}`);
      }, 2000);
    } catch (err: any) {
      ...
      if (err.name === 'UserNotFoundException') {
        ...
        friendlyMessage = 'このアカウントはメール認証が完了していないか、連絡先情報が登録されていません。お手数ですが、新しくサービス出品者登録をやり直してください。';
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            サービス出品者アカウントに登録されているメールアドレスを入力してください。
            <br />
        ...
        <div className="mt-6 text-center space-y-2">
          <Link href="/login/instructor" className="text-green-600 hover:text-green-700 font-semibold block">
            ログイン画面に戻る
          </Link>
