...
function VerifyPageContent() {
  ...
  const role = params.get("role") || "user"; // デフォルトは 'user'
  ...
  // フォーカス時のボーダー色
  const focusBorderColor = isInstructor
    ? "focus:border-green-600"
    : "focus:border-purple-600";

  // メインボタンのグラデーション
  const buttonGradient = isInstructor
    ? "bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-700 hover:to-emerald-600"
    : "bg-gradient-to-r from-purple-600 to-pink-500 hover:from-purple-700 hover:to-pink-600";

  // 再送信ボタンのスタイル
  const resendButtonStyle = isInstructor
    ? "border-green-300 text-green-700 hover:bg-green-50 hover:border-green-400"
    : "border-purple-300 text-purple-700 hover:bg-purple-50 hover:border-purple-400";

  // リンクの色
  const linkColor = isInstructor
    ? "text-green-600 hover:text-green-700"
    : "text-purple-600 hover:text-purple-700";

  // ログインページへのリンク先
  const loginHref = isInstructor ? "/login/instructor" : "/login/user";
  ...
  async function handleVerify(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const roleLabel = isInstructor ? "サービス出品者" : "ユーザー";
      setSuccessMessage(`✅ アカウント確認が完了しました！${roleLabel}ログイン画面に移動します...`);
      ...
      >
        ...
        <form onSubmit={handleVerify} className="space-y-5">
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 border-2 border-gray-200 rounded-lg ${focusBorderColor} focus:outline-none transition-colors text-center text-2xl tracking-widest`}
              placeholder="000000"
              maxLength={6}
              required
              autoComplete="off"
          ...
          <button
            ...
            className={`w-full ${buttonGradient} text-white py-3 rounded-lg font-semibold transition-all duration-300 ${
              isSubmitting || !code
        ...
        <div className="mt-6">
          <button
            ...
            className={`w-full px-4 py-2 border-2 ${resendButtonStyle} rounded-lg font-semibold transition-all duration-300 ${
              isResending || !email
                ? "opacity-50 cursor-not-allowed"
                : ""
            }`}
        ...
        <div className="mt-6 text-center">
          ...
          <div className="flex justify-center space-x-4 text-sm">
            <Link href={loginHref} className={`${linkColor} font-semibold`}>
              ログインページへ
            </Link>
