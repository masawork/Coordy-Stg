...
各ロールは**自身のエリアにのみ**アクセス可能です。誤ったエリアにアクセスした場合は、適切なページへリダイレクトされます。

| アクセス先 | 未ログイン | userログイン | instructorログイン | adminログイン |
|-----------|-----------|-------------|-------------------|---------------|
| `/user/*` | `/` へ | ✅ アクセス可 | `/instructor` へ | `/manage/admin` へ |
| `/instructor/*` | `/` へ | `/user` へ | ✅ アクセス可 | `/manage/admin` へ |
| `/manage/*` | `/manage/login` へ | `/manage/login` へ | `/manage/login` へ | ✅ アクセス可 |
...
1. **未ログイン時**:
   - `/user/*`, `/instructor/*` → トップページ `/` へリダイレクト
   - `/manage/*` → 管理者ログインページ `/manage/login` へリダイレクト

2. **別ロールでログイン中**:
   - 自身のダッシュボードへ自動リダイレクト
   - 例: instructorが`/user`にアクセス → `/instructor`へリダイレクト

3. **管理者の特権なし**:
   - 管理者でも一般ユーザー/インストラクター画面には直接アクセス不可
   - 管理者エリア `/manage/admin` でユーザー・サービス管理を行う

### 設計方針

- **ロール分離**: 各ロールは自身のエリアにのみアクセス可能
- **シームレスなリダイレクト**: 誤ったエリアへのアクセスは適切なページへ自動リダイレクト
- **未ログイン時の導線**:
  - ユーザー/インストラクターエリア → トップページへ（サービス紹介・CTA表示）
  - 管理エリア → 管理者専用ログインページへ

詳細なフロー図とシナリオは `DOCS/flows/login-flow.md` のセクション10を参照してください。
...
> **重要**: フロントエンド経由で `admin` ロールをサインアップすることはできません（`lib/auth/cognito.ts` の `registerUser` で拒否）。必ず Cognito コンソールから `ADMINS` グループに所属させてください。
...
Cognito コンソールでユーザーを作成した場合、初回ログイン時に**強制パスワード変更（FORCE_CHANGE_PASSWORD / NEW_PASSWORD_REQUIRED）**が必要になることがあります。

1. `/manage/login` にアクセス
2. 作成した管理者のメールアドレスと一時パスワードでログイン
3. **「パスワード変更が必要です」**という画面が表示された場合:
   - 新しいパスワードを入力（8文字以上、大小英字・数字を含む）
   - 確認用に再度同じパスワードを入力
   - 「パスワードを設定」をクリック
4. `/manage/admin`（管理者ダッシュボード）に遷移することを確認

> **注意**: 初回ログイン時に「パスワード変更」画面が表示されるのは正常な動作です。Cognito のセキュリティ機能として、コンソールで作成したユーザーは初回ログイン時に自分でパスワードを設定する必要があります。

#### 4. パスワード変更をスキップする方法（オプション）

Cognito コンソールで初回パスワード変更をスキップしたい場合:

1. ユーザープール → ユーザーを選択
2. 「アクション」→「パスワードを変更」
3. 「永続的」を選択して新しいパスワードを設定
4. これにより、ユーザーは設定されたパスワードでそのままログイン可能

### 管理者ログインフロー

1. `/manage/login` にアクセス
2. 管理者のメールアドレスとパスワードを入力
3. ログイン成功後、`/manage/admin` に自動リダイレクト
4. 初回ログイン時はパスワード変更画面が表示される（上記参照）

### 管理者以外のロールでアクセスした場合

- `user` / `instructor` ロールでログイン中に `/manage/admin` にアクセスした場合:
  - `/manage/login?next=/manage/admin` にリダイレクト
  - 管理者ログインフォームが表示される（別ロールでのログインを許可）
- 未ログイン時に `/manage/admin` にアクセスした場合:
  - `/manage/login?next=/manage/admin` にリダイレクト
...
| `/manage/admin` | 管理者ダッシュボード |
| `/manage/admin/dashboard` | 統計ダッシュボード |
| `/manage/admin/users` | ユーザー管理 |
| `/manage/admin/services` | サービス管理 |
| `/manage/admin/pending-charges` | 銀行振込承認 |
| `/manage/admin/identity-documents` | 本人確認書類審査 |
| `/manage/admin/settings` | 管理者設定 |
| `/admin/*` | 旧パス（`/manage/admin/*` へリダイレクト） |
